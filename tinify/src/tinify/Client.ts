...
export default class Client {
  ...
  constructor(key: string, appIdentifier?: string, proxy?: string) {
    ...
    this.userAgent = [klass.USER_AGENT, appIdentifier]
      .filter(Boolean)
      .join(" ");
  ...
  request(
    ...
    const klass = this.constructor as typeof Client;
    const options: https.RequestOptions = url.parse(
      url.resolve(klass.API_ENDPOINT, path)
    );
    ...
    return new Promise((resolve, reject) => {
      const exec = () => {
        ...
        request.on("error", (err) => {
          ...
          reject(
            new tinify.ConnectionError(`Error while connecting: ${err.message}`)
          );
        });
